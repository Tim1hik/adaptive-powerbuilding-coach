    ...
    <div className="grid gap-4">
      <form onSubmit={form.handleSubmit(submit)} className="grid gap-3 md:grid-cols-2">
        {["sleepQuality", "sleepDurationHours", "energy", "stress", "motivation", "soreness"].map((field) => (
          <label key={field} className="grid gap-1 text-sm">
            ...
            <input className="rounded-lg border border-border bg-input/40 px-3 py-2 text-foreground" {...form.register(field as keyof Values)} />
          </label>
        ))}
        <label className="grid gap-1 text-sm">
          <span className="text-foreground">{t("fields.painArea")}</span>
          <select className="rounded-lg border border-border bg-input/40 px-3 py-2 text-foreground" {...form.register("painArea")}>
            {["none", "shoulder", "low_back", "knee", "elbow", "hip", "other"].map((value) => <option key={value} value={value}>{t(`painArea.${value}`)}</option>)}
          </select>
        </label>
        <label className="grid gap-1 text-sm">
          <span className="text-foreground">{t("fields.painSeverity")}</span>
          <select className="rounded-lg border border-border bg-input/40 px-3 py-2 text-foreground" {...form.register("painSeverity")}>
      ...
      {result ? (
        <div className="rounded-lg border border-cyan-300/20 bg-cyan-300/10 p-4">
          ...
          <ul className="mt-2 list-disc pl-5 text-sm text-foreground">{result.notes.map((note) => <li key={note}>{t(`notes.${note}`)}</li>)}</ul>
          <p className="mt-3 text-sm text-muted-foreground">{t("safety")}</p>
        </div>
      ) : null}
    </div>
  );
