    ...
    <div className="grid gap-4">
      <div className="grid gap-3 md:grid-cols-2">
        <label className="grid gap-1 text-sm">
          <span className="text-zinc-300">{t("programDay")}</span>
          <select className="rounded-lg border border-white/10 bg-zinc-900 px-3 py-2" value={dayType} onChange={(event) => setDayType(event.target.value as TrainingDayType)}>
            {program.days.filter((item) => item.type !== "rest").map((item) => <option key={item.day} value={item.type}>{t(`days.${item.type}`)}</option>)}
          </select>
        </label>
        <label className="grid gap-1 text-sm">
          <span className="text-zinc-300">{t("exercise")}</span>
          <select className="rounded-lg border border-white/10 bg-zinc-900 px-3 py-2" value={exerciseName} onChange={(event) => setExerciseName(event.target.value)}>
        ...
        {["weightKg", "reps", "rpe"].map((field) => (
          <label key={field} className="grid gap-1 text-sm">
            <span className="text-zinc-300">{t(field)}</span>
            <input className="rounded-lg border border-white/10 bg-white/5 px-3 py-2" value={field === "weightKg" ? weightKg : field === "reps" ? reps : rpe} onChange={(event) => (field === "weightKg" ? setWeightKg(Number(event.target.value)) : field === "reps" ? setReps(Number(event.target.value)) : setRpe(Number(event.target.value)))} />
          </label>
        ))}
        <label className="grid gap-1 text-sm md:col-span-2">
          <span className="text-zinc-300">{t("notes")}</span>
          <textarea className="rounded-lg border border-white/10 bg-white/5 px-3 py-2" value={notes} onChange={(event) => setNotes(event.target.value)} />
        </label>
      </div>
      ...
      <div className="grid gap-2">
        ...
        {sessions.slice(0, 4).map((session) => (
          <div key={session.id} className="rounded-lg border border-white/10 bg-white/[0.04] p-3 text-sm text-zinc-300">
