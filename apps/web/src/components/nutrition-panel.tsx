...
export function NutritionPanel() {
  ...
  return (
    <div className="grid gap-4">
      <div className="grid gap-3 sm:grid-cols-4">
        {["calories", "proteinGrams", "fatGrams", "carbGrams"].map((keyName) => (
          <div key={keyName} className="rounded-lg border border-white/10 bg-white/[0.04] p-4">
            <p className="text-sm text-zinc-400">{t(`metrics.${keyName}`)}</p>
            <p className="mt-2 text-2xl font-semibold">{macros ? String(macros[keyName as keyof typeof macros]) : "0"}</p>
          </div>
        ))}
      </div>
      <label className="grid gap-1 text-sm">
        <span className="text-zinc-300">{t("dayType")}</span>
        <select className="rounded-lg border border-white/10 bg-zinc-900 px-3 py-2" value={dayType} onChange={(event) => setDayType(event.target.value as CarbCyclingStrategy)}>
      ...
      <p className="text-sm text-zinc-400">{t("cycling")}</p>
